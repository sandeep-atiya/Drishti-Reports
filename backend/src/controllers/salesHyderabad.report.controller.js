...
import salesHydrabadService from '../services/salesHyderabad.report.service.js';
...
/**
 * GET /api/v1/reports/sales-hyderabad
 * Query params: startDate, endDate
 */
export const getSalesHydrabadReport = asyncHandler(async (req, res) => {
  const { startDate, endDate } = req.query;
  const diffDays = dayjs(endDate).diff(dayjs(startDate), 'day');

  if (diffDays <= env.LARGE_RANGE_DAYS) {
    const sheets = await salesHydrabadService.getReport({ startDate, endDate });
  ...
  const jobId = uuidv4();
  createJob(jobId, () => salesHydrabadService.getReport({ startDate, endDate }));
...
/**
 * GET /api/v1/reports/sales-hyderabad/jobs/:jobId
 */
export const getSalesHydrabadJobStatus = asyncHandler(async (req, res) => {
