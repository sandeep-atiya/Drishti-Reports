import doctorSalesRepository from '../repositories/report/doctorSales.report.repository.js';
...
const doctorSalesService = {
  /**
   * Build a Doctor Sales report — identical logic to Sales Hyderabad
   * but restricted to agents whose username starts with 'Dr'.
   *
  ...
  getReport: async ({ startDate, endDate }) => {
    ...
    const [pgRows, groupSFLookup, msRows] = await Promise.all([
      doctorSalesRepository.getCallsByAgentCampaign({ startDate, endDate }),
      doctorSalesRepository.getGroupSFCampaignId(),
      doctorSalesRepository.getOrderStatsByAgentCampaign({ startDate, endDate }),
    ]);

    logger.info(
      `[DoctorSales] PG calls:${pgRows.length}  groupSFLookup:${groupSFLookup.length}  ` +
    ...
    // ── MSSQL-only rows for Group SF Dr agents ────────────────────────────────
    if (groupSFIds.size > 0) {
      ...
      logger.info(`[DoctorSales] Group SF MSSQL-only rows added: ${groupSFAgentMap.size}`);
    } else {
      logger.warn('[DoctorSales] Group SF campaign_Id not found — no MSSQL-only rows added.');
    }

    // ── Sort: campaign asc, then doctor asc ───────────────────────────────────
    rows.sort((a, b) => {
      const cmpCampaign = (a['Campaign'] || '').localeCompare(b['Campaign'] || '');
      if (cmpCampaign !== 0) return cmpCampaign;
      return (a["Doctor's Name"] || '').localeCompare(b["Doctor's Name"] || '');
    });
    ...
    logger.info(
      `[DoctorSales] Sheets — overall:${overall.length} unayur:${unayur_in.length} ` +
...
export default doctorSalesService;
