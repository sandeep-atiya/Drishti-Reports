...
import { useDoctorSales }             from '../hooks/useDoctorSales';
import DoctorSalesFilters              from '../components/DoctorSalesFilters';
import DoctorSalesTable                from '../components/DoctorSalesTable';
import { exportDoctorSalesToExcel }    from '../utils/doctorSalesHelpers';
...
const useSheetKpi = (rows) => useMemo(() => {
  ...
  const doctors        = new Set(rows.map((r) => r["Doctor's Name"])).size;
  ...
  return { calls, orders, verified, verifiedAmount, doctors, overallVerPct, overallOrdCon, avgTicket, avgRpc };
}, [rows]);

const DoctorSalesPage = ({ onMenuToggle }) => {
  const { data, loading, loadingMsg, error, fetchReport } = useDoctorSales();
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <div className="flex items-center h-14 px-4 sm:px-6 gap-3 max-w-screen-2xl mx-auto">
          ...
          <div className="flex-1 min-w-0">
            <div className="hidden sm:flex items-center gap-1.5 text-xs text-slate-400 mb-0.5">
              ...
              <span className="text-slate-600 font-medium">Doctor Sales</span>
            </div>
            <div className="flex items-center gap-2">
              <h1 className="text-base font-bold text-slate-800">Doctor Sales Report</h1>
              ...
              <span className="hidden sm:inline-flex items-center text-[10px] font-bold text-indigo-600 bg-indigo-50 border border-indigo-200 px-2 py-0.5 uppercase tracking-wide">
                Dr Agents
              </span>
            </div>
          </div>
          {data && (
            <button
              onClick={() => exportDoctorSalesToExcel({
      ...
      <div className="flex-1 p-4 sm:p-6 space-y-5 max-w-screen-2xl mx-auto w-full">
        <DoctorSalesFilters onFetch={fetchReport} loading={loading} />
        ...
        {!loading && data && (
          <div className="space-y-5">
            ...
            {kpi && (
              <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
                <KpiCard icon={Phone}        label="Total Calls"     color="slate"
                  ...
                  sub={`${kpi.doctors} doctor${kpi.doctors !== 1 ? 's' : ''}`}
                />
            ...
            <div className="bg-white border border-slate-200 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-slate-50/50">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-slate-700">
                    {activeTabDef?.label} — Doctor Breakdown
                  </span>
              ...
              <DoctorSalesTable rows={activeRows} isOverall={activeTabDef?.isOverall} />
            </div>
          </div>
        )}
        ...
        {!data && !loading && !error && (
          <div className="bg-white border border-slate-200 shadow-sm py-20 flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-indigo-500 mt-1 font-medium">Shows Dr agents only</p>
            </div>
          </div>
        )}
      </div>
