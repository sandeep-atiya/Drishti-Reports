...
import {
  fetchDoctorSalesReport,
  pollDoctorSalesJobUntilDone,
} from '../services/doctorSales.api';

const POLL_INTERVAL = Number(import.meta.env.VITE_POLL_INTERVAL_MS) || 3000;

export const useDoctorSales = () => {
  ...
  const fetchReport = useCallback(async ({ startDate, endDate }) => {
    ...
    try {
      const response = await fetchDoctorSalesReport({ startDate, endDate });
      ...
      if (!response.async) {
        ...
        const result = await pollDoctorSalesJobUntilDone(response.jobId, {
