...
import { formatNumber, formatCurrency, computeDoctorSalesTotals } from '../utils/doctorSalesHelpers';

// Columns used for individual campaign sheets (no Campaign column)
const COLS_SHEET = [
  { key: "Doctor's Name",        label: "Doctor's Name",       align: 'left',   minW: 180, type: 'name'     },
...
// Overall sheet adds Campaign column after Doctor's Name
const COLS_OVERALL = [
  { key: "Doctor's Name",        label: "Doctor's Name",       align: 'left',   minW: 180, type: 'name'     },
...
/* ── Main Table ─────────────────────────────────────────────────────────────── */
const DoctorSalesTable = ({ rows = [], isOverall = false }) => {
    ...
    const base = search.trim()
      ? (() => {
          ...
          return rows.filter((r) =>
            String(r["Doctor's Name"] || '').toLowerCase().includes(q) ||
  ...
  const totals   = useMemo(() => computeDoctorSalesTotals(filtered, isOverall), [filtered, isOverall]);
    ...
    <div>
      ...
      <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/60 flex items-center gap-2">
        ...
        <input
          ...
          placeholder={isOverall ? 'Search doctor or campaign…' : 'Search doctor name…'}
