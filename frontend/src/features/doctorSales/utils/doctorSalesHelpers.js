...
/**
 * Compute aggregate totals for a set of doctor rows.
 * Percentage columns are recomputed from raw sums so the footer is accurate.
 */
export const computeDoctorSalesTotals = (rows, includesCampaign = false) => {
  ...
  if (includesCampaign) {
    return { "Doctor's Name": 'TOTAL', 'Campaign': '', ...base };
  }
  return base;
};

/**
 * Export Doctor Sales report as a 5-sheet Excel workbook.
 */
export const exportDoctorSalesToExcel = ({
  columns,
  columnsOverall,
  overall,
  unayur_in,
  ...
  const makeSheet = (rows, cols, withTotals = true, includesCampaign = false) => {
    ...
    if (withTotals && sorted.length) {
      const totals = computeDoctorSalesTotals(sorted, includesCampaign);
  ...
  XLSX.writeFile(wb, `Doctor_Sales_${startDate}_to_${endDate}.xlsx`);
};
