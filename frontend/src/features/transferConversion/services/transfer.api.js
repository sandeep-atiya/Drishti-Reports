...
export const fetchTransferReport = ({ startDate, endDate }) =>
  apiClient
...
export const fetchTransferJobStatus = (jobId) =>
  apiClient
    .get(`/reports/transfer/jobs/${jobId}`)
    .then((r) => r.data);

export const pollTransferJobUntilDone = (jobId, { intervalMs = 3000, onProgress, signal } = {}) =>
  new Promise((resolve, reject) => {
    ...
    const tick = async () => {
      ...
      try {
        const result = await fetchTransferJobStatus(jobId);
        if (signal?.aborted) return;
