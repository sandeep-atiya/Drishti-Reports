...
import { formatNumber, formatCurrency, computeSalesConversionTotals } from '../utils/salesConversionHelpers';
...
const fmtCell = (type, val) => {
  // '—' sentinel from Group SF (no calls) — pass through as-is for all types
  if (val === '—') return '—';
  switch (type) {
    case 'amount':
    case 'ticket':
...
/* ── Main Table ─────────────────────────────────────────────────────────────── */
const SalesConversionTable = ({ rows = [], isOverall = false }) => {
  ...
  const totals   = useMemo(() => computeSalesConversionTotals(filtered, isOverall), [filtered, isOverall]);
...
export default SalesConversionTable;
