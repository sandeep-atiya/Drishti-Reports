...
import {
  fetchSalesConversionReport,
  pollSalesConversionJobUntilDone,
} from '../services/salesConversion.api';
...
export const useSalesConversion = () => {
  ...
  const fetchReport = useCallback(async ({ startDate, endDate }) => {
    ...
    try {
      const response = await fetchSalesConversionReport({ startDate, endDate });
      ...
      if (!response.async) {
        ...
        const result = await pollSalesConversionJobUntilDone(response.jobId, {
