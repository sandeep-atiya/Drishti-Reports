...
import { formatNumber, formatCurrency, computeDateWiseTotals } from '../utils/dateWiseHelpers';

/* ── Column definitions ──────────────────────────────────────────────────── */
const COLS = [
  { key: 'Date',                 label: 'Date',               align: 'left',   minW: 100, type: 'date'     },
  { key: 'Agents',               label: 'Agent',              align: 'left',   minW: 160, type: 'name'     },
  ...
  { key: 'Verified Amount',      label: 'Verified Amount',    align: 'right',  minW: 145, type: 'amount'   },
  { key: 'Verified Ticket Size', label: 'Verified Ticket Size', align: 'right', minW: 155, type: 'ticket'  },
  { key: 'Order Con',            label: 'Order Con',          align: 'center', minW: 105, type: 'pct'      },
  { key: 'Verified Con',         label: 'Verified Con',       align: 'center', minW: 115, type: 'pct'      },
...
/**
 * Format date string "YYYY-MM-DD" → "01-Mar-2024"
 */
...
const fmtCell = (type, val) => {
  switch (type) {
    case 'date':     return fmtDateDisplay(val);
    case 'amount':   return formatCurrency(val);
    case 'ticket':   return formatCurrency(val);
...
const DateWiseTable = ({ rows = [] }) => {
    ...
    const base = search.trim()
      ? (() => {
          ...
          return rows.filter((r) =>
            String(r['Agents'] || '').toLowerCase().includes(q) ||
            String(r['Date']   || '').toLowerCase().includes(q)
          );
        })()
      : rows;

    // Always keep agent-first, then date-ascending order
  ...
  const totals   = useMemo(() => computeDateWiseTotals(filtered), [filtered]);
    ...
    <div>
      ...
      <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/60 flex items-center gap-2">
        ...
        <input
          ...
          placeholder="Search agent name or date…"
                ...
                {COLS.map((col) => (
                  <td
                    ...
                    className={`px-4 py-3 whitespace-nowrap ${ac(col)} ${
                      col.type === 'date'     ? 'font-mono text-xs font-semibold text-slate-600'               :
                ...
                {COLS.map((col) => (
                  <td
                    ...
                    className={`px-4 py-3.5 whitespace-nowrap font-bold ${ac(col)} ${
                      col.type === 'date'     ? 'text-xs font-black uppercase tracking-widest text-slate-800'  :
