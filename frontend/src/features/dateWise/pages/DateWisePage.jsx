...
import {
  ...
  BarChart2, Menu, AlertCircle, FileSpreadsheet, TrendingUp,
} from 'lucide-react';
import { useDateWise }             from '../hooks/useDateWise';
import DateWiseFilters             from '../components/DateWiseFilters';
import DateWiseTable               from '../components/DateWiseTable';
import { exportDateWiseToExcel }   from '../utils/dateWiseHelpers';

/* ── Number abbreviation (Indian numbering system) ───────────────────────── */
const abbr = (n) => {
  if (!n && n !== 0) return '—';
    ...
    <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
      {['blue','emerald','amber','slate','purple','sky'].map((c) => (
        <div key={c} className="bg-white border border-slate-200 shadow-sm overflow-hidden h-24">
          <div className={`h-1 bg-${c}-200`} style={{ backgroundColor: `var(--${c}-200, #e2e8f0)` }} />
...
/* ── Main Page ─────────────────────────────────────────────────────────────── */
const DateWisePage = ({ onMenuToggle }) => {
  const { data, loading, loadingMsg, error, fetchReport } = useDateWise();

  /* Aggregate KPIs from all rows */
  const kpi = useMemo(() => {
    ...
    const agents         = new Set(r.map((row) => row['Agents'])).size;
    const days           = new Set(r.map((row) => row['Date'])).size;
    ...
    return {
      calls, orders, verified, verifiedAmount,
      agents, days,
      overallVerPct, overallOrdCon, avgRpc, avgTicket,
    };
  }, [data]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <div className="flex items-center h-14 px-4 sm:px-6 gap-3 max-w-screen-2xl mx-auto">
          ...
          <div className="flex-1 min-w-0">
            <div className="hidden sm:flex items-center gap-1.5 text-xs text-slate-400 mb-0.5">
              <span>Reports</span>
              <span>/</span>
              <span className="text-slate-600 font-medium">Date Wise</span>
            </div>
            <div className="flex items-center gap-2">
              <h1 className="text-base font-bold text-slate-800">Date Wise Report</h1>
      ...
      <div className="flex-1 p-4 sm:p-6 space-y-5 max-w-screen-2xl mx-auto w-full">
        ...
        {/* Filters */}
        <DateWiseFilters onFetch={fetchReport} loading={loading} />
        ...
        {!loading && data && (
          <div className="space-y-5">
            ...
            {kpi && (
              <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
                <KpiCard
                  icon={Phone}      label="Total Calls"      color="slate"
                  value={abbr(kpi.calls)}
                  sub={`${kpi.agents} agent${kpi.agents !== 1 ? 's' : ''} · ${kpi.days} day${kpi.days !== 1 ? 's' : ''}`}
                />
                <KpiCard
                  icon={ShoppingCart} label="Total Orders"   color="blue"
                  value={abbr(kpi.orders)}
                  sub={`Order Con: ${kpi.overallOrdCon}`}
                />
                <KpiCard
                  icon={BadgeCheck}  label="Total Verified"  color="emerald"
                  value={abbr(kpi.verified)}
                  sub={`Verification: ${kpi.overallVerPct}`}
                />
                <KpiCard
                  icon={IndianRupee} label="Verified Amount" color="amber"
                  value={abbrCurrency(kpi.verifiedAmount)}
                  sub={`Avg Ticket: ₹${abbr(kpi.avgTicket)}`}
                />
                <KpiCard
                  icon={TrendingUp}  label="Avg Ticket Size" color="sky"
                  value={`₹${abbr(kpi.avgTicket)}`}
                  sub="Verified Amt ÷ Verified Orders"
                />
                <KpiCard
                  icon={BarChart2}   label="Avg RPC"         color="purple"
            ...
            {kpi && (
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4 flex flex-wrap items-center justify-between gap-4 shadow-md shadow-blue-200">
                ...
                <div className="flex flex-wrap gap-8 text-sm">
                  <div>
                    <p className="text-blue-200 text-[11px] font-semibold uppercase tracking-wide">Total Calls</p>
                    <p className="font-bold tabular-nums">{kpi.calls.toLocaleString('en-IN')}</p>
                  </div>
                  <div>
                    <p className="text-blue-200 text-[11px] font-semibold uppercase tracking-wide">Total Orders</p>
                    <p className="font-bold tabular-nums">{kpi.orders.toLocaleString('en-IN')}</p>
                  </div>
                  <div>
                    <p className="text-blue-200 text-[11px] font-semibold uppercase tracking-wide">Verified Orders</p>
                    <p className="font-bold tabular-nums">{kpi.verified.toLocaleString('en-IN')}</p>
                  </div>
                  <div>
                    <p className="text-blue-200 text-[11px] font-semibold uppercase tracking-wide">Verified Amount</p>
                    <p className="font-bold tabular-nums">{abbrCurrency(kpi.verifiedAmount)}</p>
                  </div>
                  <div>
                    <p className="text-blue-200 text-[11px] font-semibold uppercase tracking-wide">Date Range</p>
            ...
            <div className="bg-white border border-slate-200 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-slate-50/50">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-slate-700">Agent × Date Breakdown</span>
