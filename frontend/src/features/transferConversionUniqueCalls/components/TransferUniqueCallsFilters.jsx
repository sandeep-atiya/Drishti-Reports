...
import { CalendarDays, Clock3, SlidersHorizontal, Search } from 'lucide-react';
...
const TransferUniqueCallsFilters = ({ onFetch, loading }) => {
  ...
  const pick = (p) => {
    setActiveId(p.id);
    if (p.get) { const { start, end } = p.get(); setStartDate(start); setEndDate(end); }
    else        { const t = todayStr(); setStartDate(t); setEndDate(t); }
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #edf0f7', boxShadow: '0 2px 8px rgba(0,0,0,.05)', overflow: 'hidden' }}>
        {/* header */}
        <div style={{ padding: '13px 20px', borderBottom: '1px solid #edf0f7', display: 'flex', alignItems: 'center', gap: 10, background: 'linear-gradient(135deg,#f0fdf4,#f0f9ff)' }}>
          <div style={{ width: 30, height: 30, borderRadius: 8, background: 'linear-gradient(135deg,#0891b2,#06b6d4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <CalendarDays size={14} color="#fff" />
          </div>
          <div>
            <p style={{ fontSize: 13, fontWeight: 800, color: '#0d1117', letterSpacing: '-.2px' }}>Report Filters</p>
            <p style={{ fontSize: 11, color: '#94a3b8', marginTop: 1 }}>Select period · Date & hourly ranges supported</p>
          </div>
        </div>
        ...
        <div style={{ padding: '16px 20px', display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', gap: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
              ...
              {PRESETS.map((p) => {
                ...
                return (
                  <button key={p.id} type="button" onClick={() => pick(p)} style={{
                    ...
                    border: act ? 'none' : `1px solid ${isHourly ? '#fed7aa' : '#e2e8f0'}`,
                    background: act
                      ? isHourly ? 'linear-gradient(135deg,#ea580c,#f97316)' : 'linear-gradient(135deg,#0891b2,#06b6d4)'
                      : isHourly ? '#fff7ed' : '#f8fafc',
                    color: act ? '#fff' : isHourly ? '#7c2d12' : '#4a5568',
                    fontSize: 12, fontWeight: 700, cursor: 'pointer',
                    boxShadow: act ? (isHourly ? '0 2px 8px rgba(234,88,12,.3)' : '0 2px 8px rgba(8,145,178,.3)') : 'none',
            ...
            {/* Custom date inputs */}
            {isCustomDate && (
              <div style={{ marginTop: 12, padding: '12px 14px', background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 10, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10 }}>
                <SlidersHorizontal size={13} color="#0891b2" />
                <span style={{ fontSize: 11, fontWeight: 700, color: '#0891b2' }}>Custom Date Range</span>
                  ...
                  {[
                    ...
                    <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <input type="date" value={val} min={min} max={max} onChange={(e) => set(e.target.value)}
                        style={{ height: 32, padding: '0 10px', borderRadius: 7, border: '1px solid #bae6fd', background: '#fff', fontSize: 12, color: '#0d1117', fontFamily: 'inherit', outline: 'none' }} />
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Custom hourly inputs */}
            {isCustomHour && (
              <div style={{ marginTop: 12, padding: '12px 14px', background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 10, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10 }}>
                <Clock3 size={13} color="#ea580c" />
                <span style={{ fontSize: 11, fontWeight: 700, color: '#ea580c' }}>Custom Hour Range</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  {[{ label: 'From', val: startDatetime, set: setStartDatetime }, { label: 'To', val: endDatetime, set: setEndDatetime }].map(({ label, val, set }) => (
                    <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontSize: 11, color: '#64748b', fontWeight: 600 }}>{label}</span>
                      <input type="datetime-local" value={val} onChange={(e) => set(e.target.value)}
                        style={{ height: 32, padding: '0 10px', borderRadius: 7, border: '1px solid #fed7aa', background: '#fff', fontSize: 12, color: '#0d1117', fontFamily: 'inherit', outline: 'none' }} />
                    </div>
                  ))}
                </div>
              </div>
          ...
          {badge && (
            <div style={{ flexShrink: 0 }}>
              ...
              <div style={{
                ...
                background: isCustomHour ? '#fff7ed' : '#f0f9ff',
                border: `1px solid ${isCustomHour ? '#fed7aa' : '#bae6fd'}`,
              }}>
                {isCustomHour ? <Clock3 size={12} color="#ea580c" /> : <CalendarDays size={12} color="#0891b2" />}
                <span style={{ fontSize: 12, fontWeight: 700, color: isCustomHour ? '#ea580c' : '#0891b2', fontVariantNumeric: 'tabular-nums' }}>{badge}</span>
              </div>
            </div>
          )}
          ...
          <div style={{ flexShrink: 0 }}>
            ...
            <button type="submit" disabled={loading || !canSubmit} style={{
              ...
              background: (canSubmit && !loading) ? 'linear-gradient(135deg,#0891b2,#06b6d4)' : '#e2e8f0',
              color: (canSubmit && !loading) ? '#fff' : '#94a3b8',
              fontSize: 13, fontWeight: 700, cursor: (canSubmit && !loading) ? 'pointer' : 'not-allowed',
              display: 'flex', alignItems: 'center', gap: 8,
              boxShadow: (canSubmit && !loading) ? '0 4px 14px rgba(8,145,178,.35)' : 'none',
              transition: 'all .15s',
            }}>
              {loading
                ? <><div style={{ width: 14, height: 14, border: '2px solid rgba(255,255,255,.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'uqSpin .7s linear infinite', flexShrink: 0 }} />Generating…</>
      ...
      <style>{`@keyframes uqSpin{to{transform:rotate(360deg)}}`}</style>
    </form>
  );
};
