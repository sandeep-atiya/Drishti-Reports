...
import { useTransferConversionUniqueCalls }  from '../hooks/useTransferConversionUniqueCalls';
import TransferUniqueCallsFilters             from '../components/TransferUniqueCallsFilters';
import TransferUniqueCallsTable               from '../components/TransferUniqueCallsTable';
import { exportTransferUniqueCallsToExcel }   from '../utils/transferUniqueCallsHelpers';
...
/* ── Main Page ──────────────────────────────────────────────────────────── */
const TransferConversionUniqueCallsPage = ({ onMenuToggle }) => {
  const { data, loading, loadingMsg, error, fetchReport } = useTransferConversionUniqueCalls();

  /* Aggregate KPIs from all process rows */
  const kpi = (() => {
    ...
    return { calls, transfers, orders, tCon, pCount };
  })();
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <div className="flex items-center h-14 px-4 sm:px-6 gap-3 max-w-screen-2xl mx-auto">
          ...
          <div className="flex-1 min-w-0">
            <div className="hidden sm:flex items-center gap-1.5 text-xs text-slate-400 mb-0.5">
              <span>Reports</span>
              <span>/</span>
              <span className="text-slate-600 font-medium">Transfer Conversion Unique Calls</span>
            </div>
            <div className="flex items-center gap-2">
              <h1 className="text-base font-bold text-slate-800">Transfer Conversion Unique Calls</h1>
          ...
          {data && (
            <button
              onClick={() => exportTransferUniqueCallsToExcel({
      ...
      <div className="flex-1 p-4 sm:p-6 space-y-5 max-w-screen-2xl mx-auto w-full">
        ...
        {/* Filters */}
        <TransferUniqueCallsFilters onFetch={fetchReport} loading={loading} />
            ...
            <div className="bg-white border border-slate-200 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-slate-50/50">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-slate-700">Process Report</span>
              ...
              <TransferUniqueCallsTable rows={data.campaignReport} />
            </div>
...
export default TransferConversionUniqueCallsPage;
