...
import { fetchTransferUniqueCallsReport, pollTransferUniqueCallsJobUntilDone } from '../services/transferUniqueCalls.api';

const POLL_INTERVAL = Number(import.meta.env.VITE_POLL_INTERVAL_MS) || 3000;

export const useTransferConversionUniqueCalls = () => {
  ...
  const fetchReport = useCallback(async ({ startDate, endDate }) => {
    ...
    try {
      const response = await fetchTransferUniqueCallsReport({ startDate, endDate });
      ...
      const result = await pollTransferUniqueCallsJobUntilDone(response.jobId, {
