...
import {
  fetchDateWiseCampaignReport,
  pollDateWiseCampaignJobUntilDone,
} from '../services/dateWiseCampaign.api';

const POLL_INTERVAL = Number(import.meta.env.VITE_POLL_INTERVAL_MS) || 3000;

export const useDateWiseCampaign = () => {
  ...
  const fetchReport = useCallback(async ({ startDate, endDate }) => {
    ...
    try {
      const response = await fetchDateWiseCampaignReport({ startDate, endDate });
      ...
      if (!response.async) {
        ...
        const result = await pollDateWiseCampaignJobUntilDone(response.jobId, {
