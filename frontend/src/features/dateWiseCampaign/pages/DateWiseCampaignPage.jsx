...
import {
  ...
  BarChart2, Menu, AlertCircle, FileSpreadsheet, TrendingUp, Layers,
} from 'lucide-react';
import { useDateWiseCampaign }                  from '../hooks/useDateWiseCampaign';
import DateWiseCampaignFilters                  from '../components/DateWiseCampaignFilters';
import DateWiseCampaignTable                    from '../components/DateWiseCampaignTable';
import { exportDateWiseCampaignToExcel }        from '../utils/dateWiseCampaignHelpers';
...
const DateWiseCampaignPage = ({ onMenuToggle }) => {
  const { data, loading, loadingMsg, error, fetchReport } = useDateWiseCampaign();
  ...
  const kpi = useMemo(() => {
    ...
    const campaigns      = new Set(r.map((row) => row['Campaign'])).size;
    ...
    return { calls, orders, verified, verifiedAmount, campaigns, days, overallVerPct, overallOrdCon, avgRpc, avgTicket };
  }, [data]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <div className="flex items-center h-14 px-4 sm:px-6 gap-3 max-w-screen-2xl mx-auto">
          ...
          <div className="flex-1 min-w-0">
            <div className="hidden sm:flex items-center gap-1.5 text-xs text-slate-400 mb-0.5">
              ...
              <span className="text-slate-600 font-medium">Date Wise Campaign</span>
            </div>
            <div className="flex items-center gap-2">
              <h1 className="text-base font-bold text-slate-800">Date Wise Campaign Report</h1>
          ...
          {data && (
            <button
              onClick={() => exportDateWiseCampaignToExcel({
      ...
      <div className="flex-1 p-4 sm:p-6 space-y-5 max-w-screen-2xl mx-auto w-full">
        <DateWiseCampaignFilters onFetch={fetchReport} loading={loading} />
            ...
            {kpi && (
              <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
                <KpiCard icon={Phone}       label="Total Calls"      color="slate"
                  ...
                  sub={`${kpi.campaigns} campaign${kpi.campaigns !== 1 ? 's' : ''} · ${kpi.days} day${kpi.days !== 1 ? 's' : ''}`}
                />
            ...
            <div className="bg-white border border-slate-200 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-slate-50/50">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-slate-700">Campaign × Date Breakdown</span>
              ...
              <DateWiseCampaignTable rows={data.rows} />
            </div>
          </div>
        )}
        ...
        {!data && !loading && !error && (
          <div className="bg-white border border-slate-200 shadow-sm py-20 flex flex-col items-center gap-4">
            <div className="w-16 h-16 bg-slate-100 flex items-center justify-center">
              <Layers size={30} className="text-slate-400" />
            </div>
...
export default DateWiseCampaignPage;
