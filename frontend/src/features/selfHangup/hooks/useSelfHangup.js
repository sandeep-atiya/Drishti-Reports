...
import {
  fetchSelfHangupReport,
  pollSelfHangupJobUntilDone,
} from '../services/selfHangup.api';

const POLL_INTERVAL = Number(import.meta.env.VITE_POLL_INTERVAL_MS) || 3000;

export const useSelfHangup = () => {
  ...
  const fetchReport = useCallback(async ({ startDate, endDate, campaignName }) => {
    ...
    try {
      const response = await fetchSelfHangupReport({ startDate, endDate, campaignName });
      if (controller.signal.aborted) return;

      if (!response.async) {
        // Synchronous response — data is already in the response
        setData(response);
      } else {
        // Async — poll until complete
        setLoadingMsg('Querying database — this may take a moment…');
        const result = await pollSelfHangupJobUntilDone(response.jobId, {
