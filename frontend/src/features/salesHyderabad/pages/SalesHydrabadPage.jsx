...
import { useSalesHyderabad }             from '../hooks/useSalesHyderabad';
import SalesHydrabadFilters              from '../components/SalesHydrabadFilters';
import SalesHydrabadTable                from '../components/SalesHydrabadTable';
import { exportSalesHydrabadToExcel }    from '../utils/salesHydrabadHelpers';
...
const SalesHydrabadPage = ({ onMenuToggle }) => {
  const { data, loading, loadingMsg, error, fetchReport } = useSalesHyderabad();
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <div className="flex items-center h-14 px-4 sm:px-6 gap-3 max-w-screen-2xl mx-auto">
          ...
          <div className="flex-1 min-w-0">
            <div className="hidden sm:flex items-center gap-1.5 text-xs text-slate-400 mb-0.5">
              ...
              <span className="text-slate-600 font-medium">Sales Hyderabad</span>
            </div>
            <div className="flex items-center gap-2">
              <h1 className="text-base font-bold text-slate-800">Sales Hyderabad Report</h1>
              ...
              <span className="hidden sm:inline-flex items-center text-[10px] font-bold text-orange-600 bg-orange-50 border border-orange-200 px-2 py-0.5 uppercase tracking-wide">
                HYD Agents
              </span>
            </div>
          </div>
          {data && (
            <button
              onClick={() => exportSalesHydrabadToExcel({
      ...
      <div className="flex-1 p-4 sm:p-6 space-y-5 max-w-screen-2xl mx-auto w-full">
        <SalesHydrabadFilters onFetch={fetchReport} loading={loading} />
        ...
        {!loading && data && (
          <div className="space-y-5">
            ...
            <div className="bg-white border border-slate-200 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-slate-50/50">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-slate-700">
                    {activeTabDef?.label} — HYD Agent Breakdown
                  </span>
              ...
              <SalesHydrabadTable rows={activeRows} isOverall={activeTabDef?.isOverall} />
            </div>
          </div>
        )}
        ...
        {!data && !loading && !error && (
          <div className="bg-white border border-slate-200 shadow-sm py-20 flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-orange-500 mt-1 font-medium">Shows HYD agents only</p>
            </div>
          </div>
        )}
      </div>
...
export default SalesHydrabadPage;
